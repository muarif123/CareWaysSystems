...
import { sendedcar } from '../../redux/slice';
import { useRouter } from 'next/navigation';

const Edcar = () => {
   
    const [edirpro, setedirpro] = useState({})
    ...
    useEffect(()=>{
        axios.get(`https://node.cws.com.pk/api/getonecar/${carId}`)
        .then((res)=>{
    ...
    function submitdata(){
            ...
            formdata.append('carimage',edirpro.carimage)
            dispatch(sendedcar({formdata,index:edirpro._id}))
    ...
    function editer(e){
        if(e.target.type==='file'){
            setedirpro({
                ...edirpro,[e.target.name]:e.target.files[0]
            })
        }
  ...
  return (
    <div className='py-10 dark:bg-customGray'>
      ...
      <ToastContainer position='top-center' transition={Bounce} autoClose={4000} />
...
<div className="rounded-lg border dark:bg-customGray border-gray-300 mx-auto bg-card text-card-foreground shadow-sm w-full max-w-lg" data-v0-t="card">
  ...
  <div className="p-6 flex gap-4 flex-col">
    <div className="flex flex-col gap-1.5">
      ...
      <input onChange={(e)=>editer(e)} name='carimage'
