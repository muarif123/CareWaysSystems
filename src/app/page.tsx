...
import Loaders from "../app/Compo/loader/page"

import Image from 'next/image'

import { getproducts } from './Compo/redux/slice'
import { MdEdit } from "react-icons/md"
import Feat from "../app/Compo/featured/page"
import Carousels from "./Compo/carousel/page"
...
const Home = () => {
  ...
  const { products } = useSelector((state: any) => state.add)
 
  const dispatch: any = useDispatch()
  useEffect(() => {
    dispatch(getproducts())
  }, [products])
  function displayer(itemId:any){
     ...
     <div className="container space-y-10 xl:space-y-16">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-2">
         <div className="w-full md:w-auto">
           {products.slice(9, 10).map((item: any, index: any) => {
             return (
               <>
                 <div onClick={()=>displayer(item._id)} className='w-full overflow-hidden rounded-xl mn'>
         ...
         <div className="w-full md:w-auto">
           {products.slice(1, 2).map((item: any, index: any) => {
             return (
               <>
                <div onClick={()=>displayer(item._id)} className='w-full overflow-hidden rounded-xl mn'>
                   <div style={{ overflow: "hidden" }} className="dark:hover:border-blue-600 border border-gray-200 rounded-lg p-6  dark:border-gray-800">
         ...
         <div className="w-full md:w-auto">
           {products.slice(2, 3).map((item: any, index: any) => {
             return (
               <>
                <div onClick={()=>displayer(item._id)} className='w-full overflow-hidden rounded-xl mn'>
                   <div style={{ overflow: "hidden" }} className="dark:hover:border-blue-600 border border-gray-200 rounded-lg p-6  dark:border-gray-800">
         ...
         <div className="w-full md:w-auto">
         {products.slice(4, 5).map((item: any, index: any) => {
             return (
               <>
                <div onClick={()=>displayer(item._id)} className='w-full overflow-hidden rounded-xl mn'>
                   <div style={{ overflow: "hidden" }} className="dark:hover:border-blue-600 border border-gray-200 rounded-lg p-6  dark:border-gray-800">
